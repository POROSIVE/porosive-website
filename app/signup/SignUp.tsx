         ...
         className="my-auto w-full flex flex-col gap-3 text-base font-medium">
          ...
          <div className="w-full flex flex-col text-base font-medium">
            ...
            <input 
              ...
              className="p-2 w-full text-lg text-(--subFG) border border-solid border-black rounded-lg"
          ...
          <div className="w-full flex flex-col text-base font-medium">
            <label
              className="py-2 w-full text-lg font-bold text-(--subFG)"
            >
              Email
            </label>
            <input 
          ...
          <div className="w-full flex justify-between text-base font-medium">
            ...
            <button
              className="px-6 py-2 text-center text-(--subFG) font-bold border border-solid border-black rounded-lg transition-colors hover:text-(--foreground) hover:bg-(--subBG)"
              type="submit" disabled={loading}>
                {loading ? 'Creating account...' : 'Sign up'}
            </button>
          </div>
          {message && <p className="p-2 w-full text-lg hover:text-(--accent) font-semibold">{message}</p>}
        </form>
      </div>
